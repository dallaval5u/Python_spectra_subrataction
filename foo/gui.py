...
guxxi_logger = logging.getLogger("foo.gui")  # Creating child logger
matplotlib.use('TkAgg')  # Configuring matplotlib interaction with tkinter
style.use('ggplot')  # Configuring matplotlib style

logo_img_file = os.path.dirname(os.path.abspath(__file__)) + "/Flag.png"
BG = "#ececec"
...
class BrowserPanel(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.app_quit_button = ttk.Button(self.buttons_panel,
                                          text='Quit App', command=self._quit)
    ...
    def _ask_load_specs_file(self):
        self.winfo_toplevel().load_file(file_type=datahandler.DATA_FILE_TYPES[2])
        ...
        if regions:
            ...
            for region in regions:
                if not add_dimension or not region.is_add_dimension():
                    ...
                    offset += x_offset * region.get_add_dimension_counter()
                    offset2 += y_offset * region.get_add_dimension_counter()
            if len(regions) > 1:
                ax.set_title(None)
            plotter.stylize_axes(ax)
            ax.set_ylim([ymin, ymax])
...
class CorrectionsPanel(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        blank_label.pack(side=tk.TOP, fill=tk.X)
        #MY FANCY BUTTON
        #fancy_button = ttk.Button(self, text="MY FANCY BUTTON")
        #Plot button
...
#                            regions_in_work[i] = region
    ...
    def _make_plotting_settings_subframe(self):
        ...
        binding_label = ttk.Label(energy_axis_frame, text="Kinetic", anchor=tk.W)
        ...
        kinetic_label = ttk.Label(energy_axis_frame, text="Binding", anchor=tk.W)
        ...
        #                                     )
        #####
        #self.plots_title_var_test = tk.StringVar(self, value=None)
        #self.plots_title_entry = ttk.Entry(legend_boxes, textvariable=self.plots_title_var_test, width=10)
        #self.plots_title_entry.pack(side=tk.LEFT, anchor=tk.W, expand=False)
...
class Root(tk.Tk):
    ...
    def generate_main_menu(self):
        ...
        self.main_menu_bar.add_cascade(label="Plot Options", menu=self.plot_menu)
