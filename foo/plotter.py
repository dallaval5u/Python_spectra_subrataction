...
plotter_logger = logging.getLogger("foo.plotter")  # Creating child logger
    ...
    if plot2D:
        ...
        if title:
            if region.is_sweeps_normalized():
                title_str = f"File: {region.get_info('File Name')}"
            else:
                if region.is_add_dimension():
                    title_str = f" | Sweeps: {int(region.get_info('Sweeps Number')) * region.get_add_dimension_counter()}" \
